...
class AdaptiveThresholdFocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        p_t_new = 0.1 * p_t_old + 0.9 * mean_pt
        # gamma =2
        gamma = -torch.log(p_t_new)
        # 处理大于0.5的元素
        p_t_high = torch.where(p_t > 0.7, (1.000001 - p_t)**gamma, torch.zeros_like(p_t))

        # 处理小于0.5的元素
        p_t_low = torch.where(p_t <= 0.7, (5 - p_t)**gamma, torch.zeros_like(p_t))
