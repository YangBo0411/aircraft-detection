...
class AdaptiveThresholdFocalLoss(nn.Module):
    ...
    def forward(self, pred, label,):
        ...
        p_t_new = 0.9 * p_t_old + 0.1 * mean_pt
        # gamma =2
        ...
        # 处理小于0.5的元素
        p_t_low = torch.where(p_t <= 0.5, (5- p_t) ** gamma, torch.zeros_like(p_t))

        #     # 将两部分结果相加
        modulating_factor = p_t_high + p_t_low
        loss *= modulating_factor
        return loss
        # return F.binary_cross_entropy_with_logits(pred, label, reduction='none')
...
#         """Calculates and updates confusion matrix for object detection/classification tasks."""
#         loss = F.binary_cross_entropy_with_logits(pred_score, label, reduction='none')
#
#         pred_prob = pred_score.sigmoid()  # prob from logits
#         p_t = label * pred_prob + (1 - label) * (1 - pred_prob)
#
#         p_t = torch.Tensor(p_t)  # 将张量转化为pytorch张量，使其在pytorch中可以进行张量运算
#
...
#
#         # 处理小于0.5的元素
#         p_t_low = torch.where(p_t <= 0.5, (50 - p_t) ** gamma, torch.zeros_like(p_t))
#
#         #     # 将两部分结果相加
#         modulating_factor = p_t_high + p_t_low
#         loss *= modulating_factor
...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        # self.ATFL = AdaptiveThresholdFocalLoss()
        # self.FL = FocalLoss()
        self.ATFL = AdaptiveThresholdFocalLoss()
    ...
    def __call__(self, preds, batch):
        ...
        loss[1] = self.ATFL(pred_scores, target_scores.to(dtype)).sum() / target_scores_sum
